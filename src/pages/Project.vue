<script setup lang="ts">
import Sharing from '@/components/Sharing.vue'
import ProjectNavigation from '@/components/ProjectNavigation.vue'
import ProjectText from '@/components/ProjectText.vue'
import Flipbook from '@/components/Flipbook.vue'
import Page from '@/components/Page.vue'
import video from '@/assets/videos/IMG_3967.mp4'
import CoverImage from '@/components/CoverImage.vue'
import { projects } from '@/composables/projects'
import { isMobile } from '@/composables/isMobile'
import { getImageUrl } from '@/composables/getImage'

const props = defineProps({
  projectName: { type: String }
})

const marginBottom = 'mb-6'

const project = projects.find((project) => project.title === props.projectName)
9
const flipBookImagesUrl = project?.flipbookImages?.map((image: string) => getImageUrl(image))
const coverImageUrl = getImageUrl(project?.coverImage)
</script>

<template>
  <div>
    <CoverImage
      :images="[coverImageUrl]"
      :title="project?.title"
      :class="isMobile ? 'mb-2' : 'mb-8'"
    />
    <Page>
      <template v-slot:text>
        <ProjectText
          :subtitle="project?.subTitle"
          :text="project?.text"
          :link="project?.link"
          :linkText="project?.linkText"
          :audio="project?.music"
        />
      </template>
      <template v-slot:image>
        <div>
          <v-row :class="marginBottom"><v-img :src="getImageUrl(project?.image)" /></v-row>
          <v-row v-if="project?.video" :class="marginBottom"
            ><video width="800" height="450" controls>
              <source :src="video" type="video/mp4" />
              Uw browser ondersteunt het video element niet.
            </video></v-row
          >
          <v-row v-if="project?.flipbookImages?.length > 0" :class="marginBottom"
            ><Flipbook :pages="flipBookImagesUrl"
          /></v-row>
        </div>
      </template>
      <template v-slot:footer
        ><v-row><Sharing /></v-row> <v-row><ProjectNavigation /></v-row
      ></template>
    </Page>
  </div>
</template>
